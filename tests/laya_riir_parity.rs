...
///
/// Re-armed 2026-09-26: the posture's sporadic prob-drift wobble
/// (riir-infer Issue 018, closed) was a write-after-read race in the
/// CubeCL row-softmax — the reduced max read from shared slot 0 was not
/// fenced from the sum phase's slot-0 write (fixed riir-infer `2a34bd3`).
/// Since the fix every run is bit-identical at the floor (10/10).
