...
**Status:** OPEN — Gate 0 RATIFIED 2026-09-24, LANE-ONLY scope (owner-gated delegation → reviewer verdict `#Verdict: AGREE`; the `clm-raw` ablation row dropped for now, may return after T3 on the same server boot). T1–T2 unblocked on the M3 (CPU-only); T3+ queued behind a 4090 window — see `.issues/026`. Reference design distilled 2026-09-24 (`.research/001_CLM_Contrastive_Language_Models.md`); no Rust code landed yet. The `clm` lane joins the harness as a third comparison lane (modelless · laya · clm) behind an opt-in feature; no default-build changes.
