...
**Status:** CLOSED 2026-09-24 — T1–T6 DONE; T5 (the together-republish)
landed as reflex-site `3ab373f` + worker `cde063c2` (massive 0.69 on all
three hosts, drift gate pairwise-green); T7's stale-verdict sweep landed.
Filed 2026-09-24
...
- [x] T5 — DONE 2026-09-24 (the together-republish, reflex-site `3ab373f`,
  deployed worker `cde063c2`, live-verified: massive 0.69 on m3 + m3-ane +
  4090-windows). The M3 half ran via the detached quiet-gate autofire
  (`target/t5_m3/autofire.log`): preflight PASSED twice 60 s apart
  (PROVENANCE: power=AC Power load=4.88 swap=2483.88M canary=148.5us/best5
  powermode=2(high)), harness at `2fe0be4` (~30 s), end preflight rc 1
  (load 11.82 AFTER the run — does not touch the run window). Artifact:
  `.benchmarks/023_t5_m3_modelless/` — massive **0.6900**, 13 other suites
  byte-identical (deterministic), 14 suites modelless-only.
  Publish composition (one command, 5 docs in order): primary = the fresh
  m3 metal run @8028a10 (Plan 002 T2.1), then m3-ane join (the ANE device
  column, Plan 002 T2.1), 4090-windows join (the Issue 018 full cpu run),
  then the two T5 modelless-only updates (m3 @2fe0be4, 4090 @8028a10).
  Final-state drift gate PASSED pairwise — massive 0.69 on all three
  hosts. code_fixtures is now the uniform post-laya-move population
  (nq 24, was 28 @77c408e). The superseded local-only m3+ane merge is
  preserved at reflex-site branch `archive/t2.1-local-95a177e`.
  README three-way rows were already post-023 (massive 0.6900 ¹, updated
  when 023 landed).
