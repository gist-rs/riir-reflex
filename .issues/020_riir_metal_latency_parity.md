...
- [ ] **T11 — the small-m fixed-cost band (the Bench 035 finding, the next lever).**
      Every stable remaining loss (banking77 ≈ +3–4%, massive_intent ≈ +5%,
      ag_news, sst5, emotion, xnli at +3–16%) is a SHORT-sequence,
      SINGLE-question-case suite running the LOOP path (T5 packing excludes
      1-q cases by its own measured +4.5% regression). The lever class is
      per-question fixed cost (one-CB-per-case, the two real syncs, the
      dispatch count at tiny m), not GEMM efficiency (closed, T7) and not
      attention (T10 rungs landed).
      - [-] **Rung 1 — re-price packed-at-1q POST-T7 — REFUSED on
            mechanism before building (2026-09-25, this session).** The
            Addendum-7 +4.5% is per-case MEMCPY overhead (slab copies +
            the collate pass) with nothing to amortize at n=1; at 1-q the
            packed encoder's m EQUALS the loop's m, so the band predicate
            picks the same GEMM instances for both arms and cannot flip a
            copy-dominated regression. Only the v2 packed head (which
            would CHANGE the head's copy structure) re-opens this — price
            it there, not standalone.
      - [x] **Rung 2 — per-case sync/dispatch count at small m — SIZED
            NEGATIVE 2026-09-25 (the host half), measured before building
            (the T6 discipline, one more time).** The T6 probe extended
            with the small geometries (riir-infer-laya
            `tests/metal_host_gpu_split.rs`, rows seq46/seq106 added): at
            the stable-band's own shapes the encoder host side is STILL
            1.6–1.8% of forward wall (seq46 enq 0.30 ms / sync 17.87 ms;
            seq106 0.44 / 25.06; seq188 0.62 / 34.16; seq512 1.21 / 82.87;
            quiet box, load 2.6, 9 tight rounds) — collapsing the ENTIRE
            host side (dispatch encode + chain + churn) to zero moves a
            20–66 ms case by <2%. The remaining band is therefore GPU-side
            small-m cost: thin grids (m 106 → 4 row-tiles × 16 col-tiles =
            64 TGs over 40 cores) across the ~28-layer × ~10-kernel stream,
            per-token cost disproportionate at the bottom (25 ms @ 106
            tokens vs 83 ms @ 512 — 2.4× tokens → 3.3× time). Python/MPS
            hits the same wall — which is why the band is only +3–12% and
            not +25%. The open lever class is OP-COUNT REDUCTION (kernel
      ...
      - [x] **Rung 4 — the split rule from a per-shape sweep — LANDED
            (riir-infer `ed1cb74`).** `tests/metal_splitk_shape_sweep.rs`
            (unsplit vs forced split per encoder projection, 15 rotated
            paired rounds) showed the TG ceiling was the wrong predictor:
            qkv (3072×1024) and MLP up (5248×1024) win at m ≤ 80, MLP down
            (1024×2624) at m ≤ 256, attn out at m ≤ 128. `SplitRule`
            splits every call with ≤ 3 narrow row tiles OR ≤ 64 narrow
            TGs. Whole-forward vs the first rule: seq 24–80 **0.943–0.987**
            (22–24/24), seq ≥ 106 1.000 ± 0.003. Two candidates were
            built, measured, and NOT shipped:
            - the long-k extension (MLP down to 128 TGs): an isolated-GEMM
              win that read **1.020 (0/24)** at seq 188 in the forward;
            - an xwide-tile split (64×128, each weight block read once):
              lost to narrow split on every shape — weight re-reads sit in
              L2 and are not the binding cost (the f16-B finding again).
            It also removed `DBG_SPLITK_MINM`, a bisection hack committed
            with `512477e` that read an env var on every batch-1 GEMM.
            `split_rule_matches_the_measured_sweep` pins the rule to the
            table.
      - [x] **Rung 5 — `ln_rows_wide` — LANDED, default ON (riir-infer
            `9b0e55c`).** Measured in-pipeline (62 back-to-back LN
            dispatches, one reused output), `ln_rows` at 54 rows cost
            21.7–24.6 µs against an 8.5–12.8 µs launch floor: one 32-lane
            simdgroup per row, three strided reads of the row. The wide
            kernel runs 256 threads per row with the row in registers
            (d ≤ 2048), one read, and a fixed-order threadgroup reduce —
            row-local, so packed ≡ loop holds. At 54 rows it costs 10.3–13.6
            µs (at the floor). Whole-forward paired: seq 24 **0.918**, 54
            **0.950**, 106 **0.963**, 188 **0.977**, 512 **0.995** — a win
            at every length (21–24/24). `LAYA_METAL_LN_WIDE=0` is the
            kill-switch.
      - **Still open in this lane:** folding the split reduce into the
            residual `add` — **LANDED 2026-09-26, default-OFF (riir-infer
            `b519ea0`), promotion pending the quiet-box A/B.** BOTH folds
            built (the residual fold the bullet named, plus its GLU twin
            — the MLP-up projection's reduce applies the gate directly,
            which is the LARGER traffic saving: the fused `[m × 2i]`
            staging round-trip is 5× the residual fold's m·d):
            `splitk_reduce_add` (`LAYA_METAL_FOLD_RES=1`) and
            `splitk_reduce_glu` (`LAYA_METAL_FOLD_GLU=1`), wired through
            `Backend::matmul_w_accum` / `matmul_w_glu` (unfused default
            bodies — the CPU lane and the CUDA lane are untouched), with
            the all-split predicate taking the fold, knob-off and
            mixed-plan taking the unfused stream on a dedicated grow-only
            staging buffer (the control arm keeps HEAD's allocation
            posture, never a per-call alloc). The encoder drops its two
            adds + the glu op + three scratch fields — 3 fewer dispatches
            per layer. **Bit-identity is GATED, not argued**
            (`tests/metal_fold_bits.rs`: raw f32 bits vs the unfused
            stream on the real english checkpoint at loop 24/54/80/140/
            188/317/512 + packed all-split `[30,61]` / mixed `[24,200]` /
            all-unsplit `[200,220]`, reach asserted both directions via
            `fold_dispatches()`) — 2/2 green; G5 green at BOTH postures
            (env-on 88/88, drift 3.473e-6 = the checkpoint's pre-existing
            value, top-1 1.000000); batch parity green; clippy `-D` clean
            at all three feature postures. **The promotion A/B is WRITTEN
            and WAITING on a quiet box** (`tests/metal_fold_ab.rs`,
            position-balanced paired, `AB_FOLD=res|glu|both`): the box
            carried a sibling's 8-core GPU run at load 13 all session, so
            no timing number was taken — the rungs stay default-OFF (the
            rope-hoist landing posture) until that table exists and the
            promotion call is made from its medians. ~2% ceiling per the
            sizing above stands for the residual fold; the GLU fold's
            saving is ~2.3 MB/layer of skipped round-trip at seq 54 on
            paper (~5× the residual's 0.44 MB/layer), so `AB_FOLD=glu`
            and `AB_FOLD=res` should be priced SEPARATELY before any
            both-on promotion.
...
4. **T12 (the packed-head deferred-read rung) built, gated, and NOT
   ...
   green. **PROMOTED default-on 2026-09-26 (riir-infer `40d15dd`) — the
   paired A/B resolved it:** the sequential 7-round interleave's
   ±15–35% floor was the instrument's limit, not the rung's. The new
   in-process paired harness (`tests/metal_head_defer_ab.rs`, the fold
   A/B's `with_folds` pattern through the `RiirAgent::
   set_head_defer_override` seam — one process, order alternating
   round to round, 24 paired rounds/shape, pre-registered verdict,
   AC load<6 preflight) measured: typed 5-q (seq 179/q) median on/off
   **0.984 (IQR 0.981–0.989, 24/24 wins)**, 5-q short **0.984
   (IQR 0.974–0.992, 22/24)**, 1-q wiring control FLAT **1.002**
   (both postures run the identical loop path there). Bit-identity
   inline-asserted per shape (PACKED_ACT_BITS byte-equal + identical
   Answers); packed_same_shape + packed_forward_equiv + smoke green at
   the promoted default; G5 green both postures (metal 14.0 s, cpu
   35.3 s); clippy `-D` clean at all three postures both repos.
...
5. **The priced next rung — RE-PRICED DOWN and PARKED (same follow-up,
   ...
   survive the kernel-source check. The GLU half is STRUCTURALLY
   un-foldable into a narrow epilogue: `glu_gelu_gate` pairs column j
   (gate, `gelu_as` applied) with column j+i_sz (up) — i_sz = 2624
   columns apart, and the narrow instance's BN=64 tile can never see
   both halves; the split-K fold could do it only because its reduce
   kernel reads the whole matrix anyway. Folding GLU would need a custom
   two-accumulator narrow variant (each threadgroup computing column j
   AND j+i), a real kernel rewrite with its own staging economics — not
   an epilogue arm. The residual-add half DOES fold (same (row, col)
   coordinates in the epilogue), but `add` is 1.9% of encoder GPU ≈
   4 ms/case and the fold saves the staging round-trip + dispatch ≈ 2/3
   of that ≈ **1% of case wall** — below the promotion bar the split-K
   folds cleared (−1.5…−5.9% medians) and unmeasurable against this
   box's ±15–35% floor. **SUPERSEDED the same day (2026-09-26): the
   in-process paired instrument resolved what the paper sizing could
   not — the fold A/B (`metal_fold_ab`, 24 paired rounds/shape) measured
   BOTH folds winning well past the paper bound (medians −1.5…−5.9%,
   24/24 wins in-band, the no-op control flat above the crossover) and
   the rung was PROMOTED default-on (riir-infer `53334f9`; kill-switches
   `LAYA_METAL_FOLD_RES=0` / `LAYA_METAL_FOLD_GLU=0`).** Lesson, same
   class as T12: the ±15–35% sequential floor prices the INSTRUMENT,
   not the rung — a 1–3% effect paired 24× resolves decisively.
...
The "next instrument" the status line names is LANDED:
`scripts/paired_suite_ab.sh` + `scripts/paired_suite_ab_summary.py`. Why it
exists: Bench 036 measured the publication itself at ±20–40% on
typed_decisions, and Bench 041's single sequential republish (typed·english
rust +30.6%) contradicts Bench 035's same-run table (typed·english WINNING
by 20–30%) — a single sequential run cannot adjudicate the bar because its
two lanes run minutes-to-hours apart in box time.

Design (position-balanced at suite granularity, the arena-spot A/B's law):
even-indexed suites run ONE invocation with both lanes (the harness's native
rust-then-py order); odd-indexed suites run TWO invocations in REVERSE order
(py-only via `--laya-python --skip-laya`, then rust-only). Each lane runs
first for half the suites, so drift that lands between arms lands on both
suites' comparisons in opposite directions. Every invocation writes its own
`results.json`; the summarizer pairs `laya.<ckpt>` against `py/<ckpt>` per
suite and renders the per-cell Δ (p50 AND p99, tail support beside every
p99), the tie band (`--tol`, default 2% — ms-quantization on the short
suites makes sub-2% unadjudicable), and the every-cell verdict.

```sh
LAYA_PYTHON=python3 scripts/paired_suite_ab.sh        # defaults: the 7 stable-band suites
python3 scripts/paired_suite_ab_summary.py /tmp/paired_ab_<ts>
```

Refuses without a passing `scripts/bench_preflight.sh` (the issue-021 law);
`PAIRED_AB_NO_PREFLIGHT=1` exists for plumbing validation only and stamps
the run NOT FOR PUBLICATION. Mechanically validated this session on a
LOADED box (sibling GPU job, load 13) at `--corpus-cap 8`: both directory
shapes parse, the marker propagates, the table renders — the numbers from
that run are garbage by construction and were never quoted. The real
adjudication run waits for the quiet box, same as the fold A/B.
